...
namespace questless
{
	...
	public:
		...
		Frequency(Rep cycles) : _cycles{cycles} {}

		/// @return The zero frequency for the specified representation and period.
		static Frequency<Rep, Period> zero() { return Frequency<Rep, Period>{0}; }
		...
		bool operator ==(const Frequency<Rep, Period>& right) const { return _cycles == right._cycles; }
		bool operator !=(const Frequency<Rep, Period>& right) const { return _cycles != right._cycles; }

		friend Frequency<Rep, Period> operator +(Frequency<Rep, Period> f1, Frequency<Rep, Period> f2)
		{
		...
		friend Frequency<Rep, Period> operator *(const Rep& factor, Frequency<Rep, Period> f)
		{
			return Frequency<Rep, Period>{factor * f._cycles};
		}
		friend Frequency<Rep, Period> operator *(Frequency<Rep, Period> f, const Rep& factor)
		{
			return Frequency<Rep, Period>{factor * f._cycles};
		}
		friend Rep operator *(Frequency<Rep, Period> f, const std::chrono::duration<Rep, Period> d)
		{
			return f._cycles * d.count();
		}
		friend Rep operator *(const std::chrono::duration<Rep, Period> d, Frequency<Rep, Period> f)
		{
			return f._cycles * d.count();
		}

		friend std::chrono::duration<Rep, Period> operator /(const Rep& dividend, Frequency<Rep, Period> f)
		{
			return std::chrono::duration<Rep, Period>{dividend / f._cycles};
		}
		friend Frequency<Rep, Period> operator /(Frequency<Rep, Period> f, const Rep& divisor)
		{
			return Frequency<Rep, Period>{f._cycles / divisor};
		}
		friend Frequency<Rep, Period> operator /(const Rep& dividend, const std::chrono::duration<Rep, Period> d)
		{
