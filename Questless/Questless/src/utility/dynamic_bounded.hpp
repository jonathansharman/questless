...
namespace ql {
	...
	struct dynamic_bounded : bounded<ArithmeticType, dynamic_bounded<ArithmeticType>> {
		...
		constexpr dynamic_bounded(arithmetic_type lower_bound, arithmetic_type upper_bound) noexcept
			: _lower_bound{std::move(lower_bound)}, _upper_bound{std::move(upper_bound)} {}

		//! @param value The initial value, which will be clamped to within the range.
		//! @param lower_bound The minimum value in the range (inclusive).
		//! @param upper_bound The maximum value in the range (inclusive).
		constexpr dynamic_bounded(arithmetic_type value, arithmetic_type lower_bound, arithmetic_type upper_bound) noexcept
		...
		constexpr void set_lower_bound(arithmetic_type lower_bound) noexcept {
			_lower_bound = std::move(lower_bound);
		...
		constexpr void set_upper_bound(arithmetic_type upper_bound) noexcept {
			_upper_bound = std::move(upper_bound);
