...
namespace ql::magic {
	complete teleport::cast(being& caster, gatestone& gatestone, action::cont cont) {
		return caster.agent().query_tile(queries::tile::teleport_target{},
			std::nullopt,
			action::tile_in_range_predicate(caster, _range),
			[&caster, &gatestone, cont = std::move(cont)](std::optional<region_tile::point> opt_tile_coords) {
				...
				return charge_cost{gatestone, _cost_factor * distance}.check_and_incur(
					caster, [&caster, cont = std::move(cont), tile_coords] {
