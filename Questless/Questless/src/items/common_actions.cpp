...
namespace ql {
	complete drop::perform(being& actor, cont cont) {
		actor.inventory.remove(_item.id);

		if (object* object = actor.location.region->object_at(actor.location.coords)) {
			if (item_box* old_box = dynamic_cast<item_box*>(object)) {
				...
				return actor.agent().send_message(
					queries::message::entity_in_the_way{}, [cont] { return cont(result::aborted); });

				//! @todo It should probably always be possible to drop stuff. This by itself is a good reason to allow
	...
	complete toss::perform(being& actor, cont cont) {
		//! @todo This.
