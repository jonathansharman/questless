...
namespace ql {
	...
	complete agent::aim_missile(region_tile::point /*source_coords*/, ql::being& target_being, std::function<complete(body_part*)> cont) {
	...
	complete agent::incant(gatestone& /*gatestone*/, std::function<complete(uptr<magic::spell>)> cont) {
		return cont(nullptr);
	}
	...
	complete agent::idle(action::cont cont) {
		return query_count(queries::count::wait_time{}, 10, 0, std::nullopt,
			[this, cont](std::optional<int> duration) {
				if (duration) {
