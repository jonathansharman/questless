...
namespace questless
{
	HUDController::HUDController(const Window& window) : _hud{}, _view{window}
	{}
	...
	void HUDController::update(const Input& input)
	{
		...
		if (_hud.inv_open) {
			...
			if (0 <= row && row < Inventory::Page::rows && 0 <= column && column < Inventory::Page::columns) {
				...
				if (_hud.player_being->inventory()[coords] != nullptr) {
	...
	void HUDController::player_being(Being* player_being)
	{
		_hud.player_being = player_being;
		_hud.inv_page = 0;
