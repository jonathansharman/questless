...
namespace ql {
	...
	sf::Keyboard::Key input_manager::index_to_num_key(size_t index) {
		switch (index) {
			...
			default:
				throw std::out_of_range{"Invalid number key index: " + std::to_string(index) + '.'};
		}
	}
	...
	bool input_manager::pressed(input input) const {
		return std::find(_input_event_buffer.begin(), _input_event_buffer.end(), input_event{input, true}) !=
			_input_event_buffer.end();
	}

	bool input_manager::released(input input) const {
		return std::find(_input_event_buffer.begin(), _input_event_buffer.end(), input_event{input, false}) !=
			_input_event_buffer.end();
	}

	bool input_manager::pressed(std::set<input> inputs) const {
		return std::find_if(_input_event_buffer.begin(), _input_event_buffer.end(), [&inputs](input_event ie) {
	...
	bool input_manager::press_count(std::set<input> inputs) const {
		return std::count_if(_input_event_buffer.begin(), _input_event_buffer.end(), [&inputs](input_event const& ie) {
			return !ie.pressed && inputs.find(ie.input) != inputs.end();
		});
	}
	...
	bool input_manager::any_pressed(bool include_mouse_buttons) const {
		return std::find_if(_input_event_buffer.begin(), _input_event_buffer.end(), [include_mouse_buttons](input_event ie) {
	...
	int input_manager::any_press_count(bool include_mouse_buttons) const {
		return std::count_if(_input_event_buffer.begin(), _input_event_buffer.end(), [include_mouse_buttons](input_event ie) {
			return ie.pressed && (include_mouse_buttons || std::holds_alternative<sf::Keyboard::Key>(ie.input));
		});
	}
