...
namespace sdl
{
	namespace
	{
		...
		ResourceManager<Texture> _texture_manager;
		ResourceManager<Font> _font_manager;
		ResourceManager<Sound> _sound_manager;
	}
	...
	void renderer(std::unique_ptr<Renderer> renderer)
	{
		...
		if (!initializing && _renderer != nullptr) {
			...
			_texture_manager.clear_cache();
			Renderable::refresh_all();
		}
	}

	ResourceManager<Texture>& texture_manager() { return _texture_manager; }

	ResourceManager<Font>& font_manager() { return _font_manager; }

	ResourceManager<Sound>& sound_manager() { return _sound_manager; }
}
