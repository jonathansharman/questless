...
namespace questless
{
	Human::Human(std::function<std::unique_ptr<Agent>(Being&)> agent_factory, id_t id)
		: Being(std::move(agent_factory), id, make_body(), Attributes{ATTRIBUTE_PARAMS(HumanK)})
	{}
	...
	BodyPart::ptr Human::make_body()
	{
		...
		auto head = BodyPart::make<Head>("Head", Attributes{}, false, {{-1, 10, 3, 4}});
